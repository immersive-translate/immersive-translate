(()=>{var Ja=Object.defineProperty;var Qa=(e,t)=>{for(var n in t)Ja(e,n,{get:t[n],enumerable:!0})};var S={BUILD_TIME:"2023-08-15T06:16:34.000Z",VERSION:"0.8.0",PROD:"1",REDIRECT_URL:"https://dash.immersivetranslate.com/auth-done/",BETA:"0",IMMERSIVE_TRANSLATE_INJECTED_CSS:`:root {
...
`,r="";r+=`<SAMI>${n}`,r+=`<HEAD>${n}`,r+=`<TITLE>${t.title||""}</TITLE>${n}`,r+=`<STYLE TYPE="text/css">${n}`,r+=`<!--${n}`,r+=`P { font-family: Arial; font-weight: normal; color: white; background-color: black; text-align: center; }${n}`,r+=`.LANG { Name: ${t.langName||"English"}; lang: ${t.langCode||"en-US"}; SAMIType: CC; }${n}`,r+=`-->${n}`,r+=`</STYLE>${n}`,r+=`</HEAD>${n}`,r+=`<BODY>${n}`;for(let o=0;o<e.length;o++){let a=e[o];if(a.type!=="meta"){if(!a.type||a.type==="caption"){r+=`<SYNC Start=${a.start}>${n}`,r+=`  <P Class=LANG>${kn.htmlEncode(a.text||"")}${t.closeTags?"</P>":""}${n}`,t.closeTags&&(r+=`</SYNC>${n}`),r+=`<SYNC Start=${a.end}>${n}`,r+=`  <P Class=LANG>&nbsp;${t.closeTags?"</P>":""}${n}`,t.closeTags&&(r+=`</SYNC>${n}`);continue}t.verbose}}return r+=`</BODY>${n}`,r+=`</SAMI>${n}`,r},fi=e=>/<SAMI[^>]*>[\s\S]*<BODY[^>]*>/.test(e),Ur=Se({name:Br,build:mi,detect:fi,helper:kn,parse:gi});var hi="srt",Et={toMilliseconds:e=>{let t=/^\s*(\d{1,2}):(\d{1,2}):(\d{1,2})(?:[.,](\d{1,3}))?\s*$/.exec(e);if(!t)throw new Error(`Invalid time format: ${e}`);let n=parseInt(t[1]),r=parseInt(t[2]),o=parseInt(t[3]),a=t[4]?parseInt(t[4]):0;return n*3600*1e3+r*60*1e3+o*1e3+a},toTimeString:e=>{let t=Math.floor(e/1e3/3600),n=Math.floor(e/1e3/60%60),r=Math.floor(e/1e3%60),o=Math.floor(e%1e3);return`${(t<10?"0":"")+t}:${n<10?"0":""}${n}:${r<10?"0":""}${r},${o<100?"0":""}${o<10?"0":""}${o}`}},bi=(e,t)=>{let n=[],r=t.eol||`\r
`,o=e.split(/\r?\n\s*\n/g);for(let a=0;a<o.length;a++){let u=/^(\d+)\r?\n(\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?)\s*-->\s*(\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?)\r?\n([\s\S]*)$/.exec(o[a]);if(u){let d={};d.type="caption",d.index=parseInt(u[1]),d.start=Et.toMilliseconds(u[2]),d.end=Et.toMilliseconds(u[3]),d.duration=d.end-d.start;let l=u[4].split(/\r?\n/);d.content=l.join(r),d.text=d.content.replace(/<[^>]+>/g,"").replace(/\{[^}]+\}/g,"").replace(/>>[^:]*:\s*/g,""),n.push(d);continue}t.verbose}return n},yi=(e,t)=>{let n="",r=t.eol||`\r
`;for(let o=0;o<e.length;o++){let a=e[o];if(!a.type||a.type==="caption"){n+=(a.index?a.index:o+1).toString()+r,n+=`${Et.toTimeString(a.start)} --> ${Et.toTimeString(a.end)}${r}`,n+=a.text+r,n+=r;continue}t.verbose}return n},Ti=e=>/\d+\r?\n\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?\s*-->\s*\d{1,2}:\d{1,2}:\d{1,2}(?:[.,]\d{1,3})?/.test(e),jr=Se({name:hi,build:yi,detect:Ti,helper:Et,parse:bi});var xi="sub",_n=25,vi=(e,t)=>{t.fps||=_n;let n=t.fps>0?t.fps:_n,r=[],o=t.eol||`\r
